...
@pytest.mark.parametrize(
    (
        "status",
        "expected_message",
        "expected_show_send_button",
        "expected_show_retry_button",
    ),
    [
        (None, "No acknowledgement has been sent yet.", True, False),
        ("sent", "Acknowledgement was already sent.", False, False),
        ("sending", "Acknowledgement is being sent.", False, False),
        ("send_requested", "Acknowledgement is being sent.", False, False),
        (
            "unknown",
            (
...
def test_acknowledgement_status_maps_to_display_state(
    ...
    expected_show_retry_button: bool,
) -> None:
    ...
    assert state.show_retry_button is expected_show_retry_button
...
def test_send_requested_status_hides_retry_button_customer_harm_gate() -> None:
    state = map_acknowledgement_status_to_ui_state(
        SimpleNamespace(status="send_requested"),
        has_required_order_details=True,
    )
